...
#   end
  ...
  describe 'sunday_registration_open?' do
    ...
    it 'is false when it is 7:59 on Sunday' do
      ...
      travel_to now
      result = WakeboardSet.sunday_registration_open?(set_date,now)
      expect(result).to be false
    end
...
RSpec.describe SetRider, type: :model do
  ...
  describe 'rider_joined_between method' do
    ...
    #   @user = Admin.create(email: "aramone@tamu.edu", first_name: "Axel", last_name: "Ramone")
    #   @set = WakeboardSet.create(scheduled_date: DateTime.current, driver_count: 1)
    #   @set_rider = SetRider.create( id: 1, date_registered: DateTime.current, wakeboard_set_id: @set.id, admin_id: @user.id, as_dib: true)
    #   min = DateTime.yesterday.yesterday
    #   max = DateTime.tomorrow.tomorrow
