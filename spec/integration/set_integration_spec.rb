...
RSpec.describe('Creating a set', type: :feature) do
     ...
     it 'Driver creating a valid set' do
     ...
     it 'Driver creating set with date in the past' do
          date = DateTime.current.tomorrow
          visit new_wakeboard_set_path
          fill_in 'Scheduled date', with: date
          click_on 'Create Set'
     ...
     it 'Rider viewing create button on sets page' do
          sign_out @admin
...
RSpec.describe('Joining a set', type: :feature) do
     ...
     it 'Rider joining as a dib' do
          visit wakeboard_set_path(@set.id)
          expect(find_by_id('chib-table')).not_to(have_content(@admin.full_name))
          click_on 'Join as Dib'
          visit wakeboard_set_path(@set.id)
          expect(find_by_id('chib-table')).to(have_content(@admin.full_name))
     end

     it 'Rider joining as a chib' do
          visit wakeboard_set_path(@set.id)
          expect(find_by_id('chib-table')).not_to(have_content(@admin.full_name))
     ...
     it 'Rider who is already joined as a dib trys to join as dib again' do
          visit wakeboard_set_path(1)
          expect(find_by_id('dib-table')).to(have_content(@admin.full_name))
          click_on 'Join as Dib'
          visit wakeboard_set_path(1)
          expect(find_by_id('dib-table')).to(have_content(@admin.full_name))
     end

     it 'Rider who is already joined as a dib trys to join as chib' do
          visit wakeboard_set_path(1)
          expect(find_by_id('dib-table')).to(have_content(@admin.full_name))
          click_on 'Join as Chib'
          visit wakeboard_set_path(1)
     ...
     it 'Rider who is already joined as a chib trys to join as chib again' do
          visit wakeboard_set_path(1)
          expect(find_by_id('chib-table')).to(have_content(@admin.full_name))
          click_on 'Join as Dib'
          visit wakeboard_set_path(1)
     ...
     it 'Rider who is already joined as a chib trys to join as dib' do
          visit wakeboard_set_path(1)
          expect(find_by_id('chib-table')).to(have_content(@admin.full_name))
          click_on 'Join as Chib'
          visit wakeboard_set_path(1)
          expect(find_by_id('dib-table')).not_to(have_content(@admin.full_name))
     end
end
...
RSpec.describe('Leaving a set', type: :feature) do
     ...
     it 'Dib and Chib rider leaving' do
          visit wakeboard_set_path(1)
          expect(find_by_id('chib-table')).to(have_content(@admin.full_name))
          expect(find_by_id('dib-table')).to(have_content(@admin.full_name))
          click_on 'Leave Set'
     ...
     it 'Rider who isn\'t on set leaving' do
          visit wakeboard_set_path(1)
