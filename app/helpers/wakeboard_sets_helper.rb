module WakeboardSetsHelper
  ...
  def set_available?(user_id, set, current_date = DateTime.current)
    if current_date.sunday.today?
      ...
      registration = DateTime.new(current_date.year, current_date.month, current_date.day, 20, 0, 0, current_date.zone)
      if set.scheduled_date.in_time_zone.before?(registration)
        return true
      end

      # set is after registration time
