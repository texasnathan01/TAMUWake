class UsersController < ApplicationController
     ...
     def search
          @products = Admin.where('is_approved = true AND first_name LIKE ? AND last_name LIKE ?',
     ...
     def index
          ...
          if @user.role_id == 1 || @user.role_id >= -1
               ...
               @users = if params[:first_name].present? || params[:last_name].present?
                             Admin.where('is_approved = true AND first_name LIKE ? AND last_name LIKE ?',
                                         "%#{params[:first_name]}%", "%#{params[:last_name]}%"
                             )
                        else
     ...
     def approval
          ...
          if @user.role_id == 1 || @user.role_id >= -1
               # check if user passed filter arguments when looking at user
               @users = if params[:first_name].present? || params[:last_name].present?
                             Admin.where('is_approved = false AND first_name LIKE ? AND last_name LIKE ?',
                                         "%#{params[:first_name]}%", "%#{params[:last_name]}%"
                             )
                        else
